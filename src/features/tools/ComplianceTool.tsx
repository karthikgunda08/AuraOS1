...
import { PhoenixEnginePanelProps, CodeComplianceReport } from '../../types';
             ...
             <GenericApiTool
                ...
                buildPayload={(p) => {
                    ...
                    return [
                        selectedRoomId,
                        standard,
                        { projectType: p.currentProject?.projectType || 'building', levels: p.levels, planNorthDirection: p.planNorthDirection, propertyLines: p.propertyLines, terrainMesh: p.terrainMesh, zones: [], infrastructure: [] }
                    ];
                }}
                onSuccess={(result, p) => { /* Result is handled locally by the tool */ }}
