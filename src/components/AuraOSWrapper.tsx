...
const AuraOSWrapper: React.FC = () => {
    ...
    const [activeSketcherTool, setActiveSketcherTool] = useState<SketcherTool>('select');
    const [editorMode, setEditorMode] = useState<EditorMode>('concept');
    ...
    const handleSave = async (commitMessage: string) => {
        const { currentProject, levels, zones, infrastructure, planNorthDirection, propertyLines, terrainMesh, stagingSettings, savedCameraViews, holocron } = store;
