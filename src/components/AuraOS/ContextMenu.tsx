...
const ContextMenu: React.FC<ContextMenuProps> = ({ x, y, object }) => {
    const { setContextMenu, setActiveTool, togglePanelVisibility, runAiFurnishRoom, setView, atmanSignature, deleteSelectedObject } = useAppStore();

    const handleAction = (e: React.MouseEvent, tool: string) => {
        e.stopPropagation();
        setActiveTool(tool as any);
        if (!useAppStore.getState().panelStates.phoenixEngine.isVisible) {
    ...
    const handleFurnish = (e: React.MouseEvent, roomId: string, useAtman: boolean) => {
        ...
        runAiFurnishRoom(roomId, style, useAtman);
    };
        ...
        if (!object) {
            return (
                <>
                    ...
                    <MenuItem onClick={(e) => handleAction(e, 'masterArchitect')}>🌟 New Concept...</MenuItem>
            ...
            case 'wall':
                return (
                    <>
                        ...
                        <MenuItem onClick={(e) => handleAction(e, 'structure')}>🏛️ Analyze Structure</MenuItem>
                ...
                return (
                    <>
                        ...
                        <MenuItem onClick={(e) => handleAction(e, 'render')}>🖼️ Render Room</MenuItem>
                         <MenuDivider />
                        <MenuItem onClick={handleDelete}>🗑️ Delete Room</MenuItem>
                    </>
                );
            case 'placedModel':
                 return (
                    <>
                        ...
                        <MenuItem onClick={() => setView('staging')}>Staging Engine</MenuItem>
                         <MenuDivider />
