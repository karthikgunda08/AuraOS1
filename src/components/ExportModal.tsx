...
import { SketcherHandles } from '../features/FloorPlanSketcherSection';
import { View3DHandles } from './Basic3dViewPanel';
...
export const ExportModal: React.FC<ExportModalProps> = ({ onClose, sketcherRef, view3dRef, projectName }) => {
    const { exportCometPackage } = useAppStore();
    const [isPackaging, setIsPackaging] = useState(false);

    const handleExport = (type: '2d_png' | '3d_png' | '3d_glb') => {
            ...
            >
                ...
                <div className="space-y-3">
                    ...
                    <button onClick={handleCometExport} disabled={isPackaging} className="w-full p-4 bg-gradient-to-r from-purple-600 to-indigo-700 hover:from-purple-500 hover:to-indigo-600 rounded-lg text-left transition-colors border border-purple-500 disabled:opacity-50">
                        <h3 className="font-semibold text-white flex items-center gap-2">✨ Comet Package (.zip)</h3>
                        <p className="text-xs text-slate-300">Bundle all project assets (drawings, renders, folio, BoQ) into a single download.</p>
