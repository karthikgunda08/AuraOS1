...
import { ProjectSummary, NexusReport, SvaDharmaResponse } from '../../types';
...
const ActionButton: React.FC<{ action: { title: string, cta: string }, onAction: (actionType: string) => void }> = ({ action, onAction }) => {
    return (
        <Button onClick={() => onAction(action.cta)} size="sm" variant="outline" className="text-xs">
            {action.cta}
        </Button>
    );
};
...
const ReportDisplay: React.FC<{ report: NexusReport }> = ({ report }) => {
    const { setView, setActiveTool, togglePanelVisibility } = useAppStore(state => ({
    ...
    const handleAction = (cta: string) => {
        ...
        if (ctaLower.includes('marketplace')) {
            ...
            setActiveTool('nexusAdvisor');
            togglePanelVisibility('phoenixEngine');
        }
    };
    
    return (
        <MotionDiv variants={{ hidden: { opacity: 0 }, visible: { opacity: 1, transition: { staggerChildren: 0.15 } } }} initial="hidden" animate="visible" className="space-y-4">
