...
import { User, UpdateUserPayload, Workspace, ChangePasswordPayload } from '../../types';
...
const WorkspacesTab: React.FC = () => {
    const { workspaces, applyWorkspaceLayout, saveCurrentWorkspace, deleteWorkspace } = useAppStore(state => ({
    ...
    const handleSave = () => {
        const name = prompt("Enter a name for your current workspace layout:");
        if (name) {
    ...
    return (
        <div className="space-y-6">
             <h3 className="text-xl font-semibold text-slate-100">Workspace Layouts</h3>
             <p className="text-sm text-slate-400">Save and load your favorite panel arrangements in the AuraOS editor.</p>
             <div className="space-y-2">
                {workspaces.map(ws => (
                    <div key={ws.id} className="flex items-center justify-between p-3 bg-slate-700/50 rounded-lg">
                        <span className="font-medium text-slate-200">{ws.name}</span>
                        <div className="flex gap-2">
                             <Button variant="outline" size="sm" onClick={() => applyWorkspaceLayout(ws.layout)}>Apply</Button>
                ...
                {workspaces.length === 0 && <p className="text-center text-slate-500 py-4">No saved workspaces.</p>}
             </div>
             <div>
                <Button onClick={handleSave}>Save Current Layout as New Workspace</Button>
             </div>
        </div>
    );
};
...
const ProfilePage: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'profile' | 'public_profile' | 'security' | 'workspaces'>('profile');
      ...
      >
        ...
        <div className="flex flex-grow overflow-hidden">
            <nav className="w-1/4 border-r border-slate-700 p-4 flex-shrink-0 flex flex-col gap-1">
                ...
                <NavButton tabId="workspaces">Workspaces</NavButton>
