...
import { User, AuthResponse, UpdateUserPayload, ChangePasswordPayload, UserAnalyticsData, Workspace } from '../types';
...
export const getUserWorkspaces = async (): Promise<Workspace[]> => {
    return fetchWithAuth('/auth/me/workspaces');
};

export const saveUserWorkspace = async (workspace: Omit<Workspace, 'id'>): Promise<Workspace[]> => {
    return fetchWithAuth('/auth/me/workspaces', {
        method: 'POST',
...
export const deleteUserWorkspace = async (workspaceId: string): Promise<Workspace[]> => {
    return fetchWithAuth(`/auth/me/workspaces/${workspaceId}`, {
        method: 'DELETE',
    });
};
